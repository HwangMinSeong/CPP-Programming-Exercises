...
void loadRanking(const char* ranking)
{
    FILE* fp = fopen_s(&fp, "ranking", "r");
    if (fp == NULL)
        initRanking();
    else {
        for (int i = 0; i < nMVP; i++)
...
void storeRanking(const char* ranking)
{
    FILE* fp = fopen_s(&fp, "ranking", "w");
    if (fp == NULL) return;
    for (int i = 0; i < nMVP; i++)
